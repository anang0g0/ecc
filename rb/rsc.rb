...
@h=Array.new(9){Array.new(16) {0}}
@temp=Array.new(9){Array.new(16) {0}}
@tmp=Array.new(9){Array.new(16) {0}}
@tmp2=Array.new(9){Array.new(16) {0}}
@ss=Array.new(9){Array.new(16) {0}}
lo=[32]
ev=[16]
N=16
K=8
...
err=[0,0,0,0,0,0,5,0,0,6,0,0,0,0,0]
#print @gf[@fg[err[11]]],"\n"
#exit()
for i in 0..3 #7
  for j in 0..14
    s[i]^=@gf[mlt(@fg[err[j]],@h[i][j])];
  end
print @fg[s[i]]-1,","
end
print "\n"
...
if(k3<nn)
for i in k3..3
  @tmp[k2][i]=mlt(@temp[k2][i],@temp[k2][k]);
end
for j in k3..3
  for i in 0..nn
    @tmp2[i][j]=mlt(@temp[i][k],@tmp[k2][j]);
  end
end
for j in k3..3
  for i in 0..nn
...
k3=2-k;
for i in 0..k3
  @tmp[k][i]=mlt(@temp[k][i],@temp[k][k2]);
end
for j in 0..k3
  for i in 0..nn
    @tmp2[i][j]=mlt(@temp[i][k2],@tmp[k][j]);
  end
end
for j in 0..k3
  for i in 0..nn
...
if(@temp[k][j]!=1)
  ...
  for i in 0..2
    if(i!=k)
      @temp[i][j]=mlt(@temp[i][j],a);
    end
  end
  print @temp[2][0],",;",@temp[1][0],",",@temp[0][0],"\n\n"
  for i in 0..2
    @tmp[i][j]=mlt(@temp[i][j],@temp[k][k]);
  end
  print @temp[2][0],"=",@temp[1][0],"a1+",@temp[0][0],"a2\n\n"
  for i in 0..2
    @temp[i][k]=@fg[@gf[@temp[i][k]]^@gf[@tmp[i][j]]];
  end
  print @temp[2][0],"=",@temp[1][0],"a1+",@temp[0][0],"a2\n"
...
#o=[1,5]
for x in 0..1
  o=@fg[@gf[mlt(div(@fg[w[2]],a2),div(1,xx[x]))]^@gf[div(@fg[w[3]],a2)]];

print "o=",o,"\n"
#print "xx=",div(1,xx[x]),"\n"
  l=mlt(xx[x],@fg[1^@gf[mlt(xx[1-x],div(1,xx[x]))]]);
print "l=",l,"\n"
#l=15;
  z=div(o,l);
  print @gf[z],"\n"
